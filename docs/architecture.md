...
**They bit, and the fallback shipped.** `webkitSpeechRecognition` has a failure mode that reads as a network fault and is not: outside Google Chrome the API frequently *exists* and always fails with `error: "network"`, because Chromium forks ship the interface without Google's speech backend credentials. Firefox and Safari do not implement it at all. So the browser API is tried first (instant, free, no round trip) and, on that specific failure, the app switches itself to capturing audio and posting each utterance to `POST /voice/transcribe`, which uses Groq's hosted `whisper-large-v3-turbo` on the `GROQ_API_KEY` the project already requires. No new credential, no local model download, and it works in every browser that can record audio.

**What that path got wrong first, and the rule it earned.** Knowing where an utterance ends is the thing the browser API gave away for free, and the first implementation guessed: it recorded fixed `MediaRecorder` segments and closed a clip whenever 400ms of audio had accumulated and 900ms of it was quiet — a condition an *idle* microphone meets every 1.3 seconds, forever. Four symptoms, one cause: silence uploaded on a loop, the rate limit gone in about 26 seconds so every real utterance afterwards failed, Whisper's silence filler (“Thank you.”, “you”) arriving as genuine chat messages, and the first syllable after each pause lost between segments. `apps/web/src/lib/audio.ts` replaces it with continuous PCM capture and a voice-activity detector, so audio is only ever uploaded when speech was heard. The rule worth keeping is **measure the noise floor, never hardcode it**: the original fixed `SILENCE_RMS = 0.018` was one microphone in one room, and browser auto-gain-control lifts a cheap laptop mic's idle floor above it, at which point the clip never closes at all. The corollary is that this path is **invisible from Chrome** — it only runs where the browser's own service fails — which is why `npm run vad` drives the detector with synthesised audio instead of relying on someone remembering to test in Brave.
...
**Also done (both NEW, neither in the original blueprint):**
...
5. **Wake word** — done, and keyless. `matchWakePhrase` (`apps/web/src/lib/speech.ts`) matches "hey cipher" at the start of an utterance in the transcript the recogniser is already streaming, strips it, and opens a 20-second follow-up window so a conversation does not require repeating the name every turn. Anchored to the start deliberately: this assistant is called CIPHER and says its own name often, so "I renamed the cipher module" must stay a sentence. No Picovoice account, no new dependency, no second audio pipeline. The cost is stated in Section 8: unlike a real wake-word engine, the recogniser is listening the whole time, which is why hands-free is opt-in per session and never persisted.

**What measurement changed about the plan.** Two of the three remaining items shipped differently from how they were specified above, both because live data contradicted the design:

- The galaxy was specified with "links = embedding similarity above a threshold". There is no such threshold. `scripts/memory_graph_calibrate.py --corpus`, run against real gemini-embedding-001 output, found every pair in the labelled corpus scoring between 0.641 and 0.809 — and unrelated facts ("allergic to peanuts" / "learning guitar", 0.768) sitting above what a reader would call related. Embedding models put short first-person sentences in a narrow cone, so absolute cosine similarity between two stored facts barely discriminates. The graph is therefore built from KNN (rank survives that compression) with a floor derived from each store's own mean and spread, and the UI says "nearest by similarity" rather than "related", because that is the only claim the data supports.
- Preflight earned its place immediately rather than eventually. Within minutes of existing it caught `/memory/graph` returning 503 against real Postgres — an `AmbiguousParameterError` on a nullable float bind — while all 183 unit tests passed, because the SQLite test fake reimplements that query in Python and never runs the SQL. It also now diffs the running server's OpenAPI document against the checkout, after stale processes wasted time three separate times in one session.
